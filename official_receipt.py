...
def generate_official_receipt_text(
    ...
    if str(payment.get("method", "")).strip().upper() == "CASH":
        open_drawer_command = "\x1b\x70\x00\x19\xfa"
    ...
    def build_copy(copy_type):
        ...
        # Dynamic Split Details Appended Here
        if split_data:
            ...
            # Sub-Section specifically displaying exact items
    ...
    final_receipt_text = open_drawer_command + cashier_copy + customer_copy

    try:
        folder_name = "Official_Receipts"
        os.makedirs(folder_name, exist_ok=True)

        safe_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"OR_{txn_code}_{safe_time}.txt"
